...
class CategoryService {
  static const String apiEndpoint = "http://13.125.197.66:18090/api/category"; // 서버 URL
  ...
  static Future<CategoryVo?> fetchCategoryData(int categoryId, int subCategoryId) async {
    final String url = "$apiEndpoint/sc/$categoryId/$subCategoryId"; // ✅ URL 구조 변경
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        if (response.data is List && response.data.isNotEmpty) {
          final parsedCategory = CategoryVo.fromJson(response.data[0]); // ✅ JSON을 CategoryVo로 변환
          print('✅ [JSON 파싱 성공]');
